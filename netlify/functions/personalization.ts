...
const handler: Handler = async (event, context) => {
  ...
  const path = event.path.replace(/\\/netlify\\/functions\\/personalization/, '').replace(/^\\/+/, '');
  ...
  try {
    ...
    switch (path) {
      ...
      case 'similar-entries':
        return await findSimilarEntries(event, headers);
      case 'update-feedback':
        return await updateUserFeedback(event, headers);
      case 'personalized-prompts':
        return await getPersonalizedPrompts(event, headers);
      case 'user-profile':
 ...
 */
async function updateUserProfile(event: any, headers: any) {
  try {
    const { userId, journalEntry, drawnCards, zodiacPlacements, emotionTags } = JSON.parse(event.body);
    ...
    if (journalEntry) {
      ...
      if (journalError) {
        ...
        return {
          statusCode: 500,
          headers,
          body: JSON.stringify({ error: 'Failed to store journal entry' }),
        };
      }
    }
    ...
    if (drawnCards && drawnCards.length > 0) {
      ...
      if (cardsError) {
        ...
        return {
          statusCode: 500,
          headers,
          body: JSON.stringify({ error: 'Failed to fetch user cards' }),
        };
      }
 ...
 */
async function getPersonalizedInterpretation(event: any, headers: any) {
  try {
    ...
    if (journalError) {
      ...
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ error: 'Failed to fetch journal entries' }),
      };
    }
 ...
 */
async function findSimilarEntries(event: any, headers: any) {
  try {
    const { userId, text, limit = 5 } = JSON.parse(event.body);

    if (!userId || !text) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'Missing required parameters: userId or text' }),
      };
    }
    ...
    if (error) {
      ...
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ error: 'Failed to find similar journal entries' }),
      };
    }
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ entries: similarEntries }),
    };
  } catch (error) {
    console.error('Error finding similar journal entries:', error);
    return {
 ...
 */
async function updateUserFeedback(event: any, headers: any) {
  try {
    const { userId, interpretationId, feedback, spentTimeMs, scrollDepth } = JSON.parse(event.body);

    if (!userId || !interpretationId) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'Missing required parameters: userId or interpretationId' }),
      };
    }
    ...
    if (error) {
      ...
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ error: 'Failed to store user feedback' }),
      };
    }
 ...
 */
async function getPersonalizedPrompts(event: any, headers: any) {
  try {
...
`;
    ...
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ error: 'Failed to generate personalized prompts' }),
    };
  }
}
 ...
 */
async function getUserProfile(event: any, headers: any) {
  try {
    const userId = event.queryStringParameters?.userId;
    ...
    if (feedbackError) {
      ...
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ error: 'Failed to fetch user feedback' }),
      };
    }
