  ...
  useEffect(() => {
    const loadDailyCard = async () => {
      ...
      try {
        ...
        if (dailyCard.id === "07-chariot" || dailyCard.id === "13-death" || dailyCard.id === "16-tower") {
  ...
  if (isLoading) {
    return (
      <Card className={`w-full max-w-md mx-auto ${className}`}>
        <CardHeader>
          <CardTitle className="text-center text-gold">Your Daily Tarot</CardTitle>
    ...
    return (
      <Card className={`w-full max-w-md mx-auto ${className}`}>
        ...
        <CardContent className="text-center py-10">
          <p className="mb-4 text-light/80">The mystical energies are momentarily disrupted.</p>
          <Button 
            variant="outline" 
        ...
        >
          <Card className="w-full max-w-4xl mx-auto bg-primary-10 border-gold/30">
            <CardHeader>
              ...
              <CardTitle className="text-center mt-2 text-gold">{card.name}</CardTitle>
              <CardDescription className="text-center">
                {card.arcana === 'major' ? 'Major Arcana' : `${card.suit?.charAt(0).toUpperCase()}${card.suit?.slice(1)} of ${card.arcana.charAt(0).toUpperCase()}${card.arcana.slice(1)}`}
                {isReversed && ' • Reversed'}
              </CardDescription>
            </CardHeader>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <h3 className="text-lg font-medium text-gold mb-2">Keywords</h3>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {card.element && (
                      <div>
                        <h3 className="text-sm font-medium text-gold mb-1">Element</h3>
                    ...
                    {card.zodiacSign && (
                      <div>
                        <h3 className="text-sm font-medium text-gold mb-1">Zodiac</h3>
          ...
          <Card className="w-full max-w-md mx-auto bg-primary-10 border-gold/30">
            ...
            <CardContent className="flex flex-col items-center">
              ...
              <p className="text-sm text-center mb-4 text-light/80">
                {isFlipped ? 'Tap the card again to flip' : 'Tap the card to reveal'}
              </p>
