...
export function HoroscopePreview({
  ...
  const {
    ...
    error,
    refetch,
  } = useQuery<HoroscopePreviewResponse>({
      ...
      try {
        ...
        if (error instanceof Error && error.message.includes("401")) {
          ...
          return {
            sign: "",
            date: "",
            content: "Please log in to view your horoscope.",
            message: "",
          };
        }
        throw error;
  ...
  const getSignInfo = (sign: string) => {
    return (
      zodiacSigns.find((s) => s.sign.toLowerCase() === sign) || zodiacSigns[0]
    );
  };
    ...
    return (
      <div className="bg-dark/40 backdrop-blur-sm border border-light/10 rounded-lg p-6 text-center">
        <h3 className="text-xl font-medium text-light mb-3">
          Unable to fetch horoscope: {error.message}
        </h3>
      ...
      <div className="mb-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-light/60 text-sm">
            ({signInfo.dates} •{" "}
            {signInfo.element.charAt(0).toUpperCase() +
              signInfo.element.slice(1)}
            )
          </span>
        </div>
