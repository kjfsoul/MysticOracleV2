              ...
              <div className="flex flex-col items-center relative">
                ...
                >
                  {React.cloneElement(item.icon, { 
                    className: `h-5 w-5 ${isActive ? 'text-gold' : 'text-light/70'}` 
                  })}
                </div>
          ...
          <Sheet>
            <SheetTrigger asChild>
              <Button variant="ghost" className="p-0 flex flex-col items-center">
                ...
                <span className="text-xs mt-1 font-medium text-light/70">More</span>
              </Button>
            </SheetTrigger>
            <SheetContent side="bottom" className="bg-dark/95 backdrop-blur-md border-gold/30 pt-6 pb-20">
              <SheetHeader>
              ...
              <div className="grid grid-cols-3 gap-4 mt-6">
                {[
                  ...
                  { name: 'Readings', path: '/readings', icon: <History className="h-5 w-5" /> },
                  { name: 'Settings', path: '/settings', icon: <Settings className="h-5 w-5" /> },
                  { name: 'Premium', path: '/upgrade', icon: <CreditCard className="h-5 w-5" /> },
                ].map((item) => (
                  <Link key={item.path} href={item.path}>
