...
import { horoscopeDB } from "./supabase-mock";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/auth-test", (req, res) => {
    ...
    console.log(
      "DEBUG - Auth Test Route - Is Authenticated:",
      req.isAuthenticated()
    );
    ...
    const loginLink = `<a href="/api/direct-login-test?returnTo=${encodeURIComponent(
      req.query.returnTo || "/debug-auth"
    )}">Click here to login as test user</a>`;
    ...
    req.login(
      ...
      (err) => {
        if (err) {
          ...
          if (req.headers.accept?.includes("text/html")) {
            return res.redirect(
              `/debug-auth?error=${encodeURIComponent(
      ...
      res.json({
        ...
        spreads: [
          ...
          },
          {
            ...
            name: "Zodiac Spread",
            description:
              "Twelve-card spread based on the houses of the zodiac, combining tarot and astrology",
            cardCount: 12,
          },
        ],
      });
  ...
  app.get("/api/tarot-cards/card-of-the-day", async (req, res) => {
    try {
      ...
      const cardOfDay = {
        ...
        message:
          "Sign up for a premium account to receive personalized readings and deeper insights!",
      };
  ...
  app.post("/api/public/zodiac-spread", async (req, res) => {
    try {
      ...
      const reading = {
        ...
        createdAt: new Date().toISOString(),
        message:
          "Sign up for a premium account to save this reading and get deeper insights!",
      };
  ...
  app.post("/api/astrology-charts/birth-chart", async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        validationErrors.push(
          "Birth location is required and must be at least 2 characters"
        );
      }
      ...
      console.log(
        `Creating birth chart for ${name} born on ${birthDate} at ${
      ...
      try {
        ...
        if (
          ...
          errorMessage.includes("interpretation")
        ) {
  ...
  app.get("/api/horoscopes/preview/:sign", async (req, res) => {
    try {
      ...
      if (!horoscope) {
        ...
        const { horoscope: content } = await generateDailyHoroscope(
          sign,
          false
        );
        ...
        const previewHoroscope = {
          ...
          message:
            "Sign up for full daily horoscopes and personalized insights!",
  ...
  app.get("/api/horoscopes/:sign", async (req, res) => {
    try {
      ...
      if (!horoscope) {
        console.log(
          `Generating new horoscope for ${sign}. User authenticated: ${isAuthenticated}`
        );
        const { horoscope: content, premiumInsight } =
          await generateDailyHoroscope(sign, isPremium);
      ...
      console.log(
        `Returning horoscope for ${sign}. User authenticated: ${isAuthenticated}`
      );
      res.json(horoscope);
    } catch (error) {
  ...
  app.get("/api/compatibility/:sign1/:sign2", async (req, res) => {
    try {
      ...
      if (
        !zodiacSigns.some((z) => z.sign.toLowerCase() === sign1) ||
      ...
      res
        .status(500)
        .json({ message: "Failed to generate compatibility reading" });
    }
  });
  ...
  // Commented out due to missing createCheckoutSession function
  /*
  ...
  */
  ...
  // Removed duplicate route for /api/subscription/success

  // Removed duplicate route for /api/subscription/status
